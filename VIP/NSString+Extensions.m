...
-(CGFloat)widthWithFontSize:(CGFloat)fontSize{
    return  [self boundingRectWithSize:CGSizeMake(MAXFLOAT, 21) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:fontSize]} context:nil].size.width;
}
...
{
    NSString *name = [self getFitstLetter];
    
    NSString *no = [self removeFirstLetter];
    
    return ([self isValidateProvinceWithName:name] && [self isValidateWithNo:no]);
}

- (BOOL)isValidateProvinceWithName:(NSString *)name
{
